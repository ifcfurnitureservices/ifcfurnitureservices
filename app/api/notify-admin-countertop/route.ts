...
// Tell Next.js this route should be dynamically rendered (not statically evaluated at build time)
...
export async function POST(req: Request) {
  try {
    ...
    const { data: project } = await supabaseAdmin.from('modular_projects').select('job_id, customer_name, client_id').eq('id', projectId).single();
    if (!project) throw new Error("Project missing");

    const { data: client } = await supabaseAdmin.from('clients').select('full_name, email').eq('id', project.client_id).single();
